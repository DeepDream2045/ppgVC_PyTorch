...
if paras.oneshotvc:
    ...
    if paras.bilstm:
        from bin.train_ppg2mel_oneshotvc import Solver
    else:
    ...
    print(">>> Use location sensitive attention based seq2seq model")
    from bin.train_linglf02mel_seq2seq_lsa import Solver
    mode = "train"
    solver = Solver(config, paras, mode)
    solver.load_data()
    solver.set_model()
    ...
    if paras.finetune:
        from bin.finetune_linglf02mel_seq2seq_encAddlf0 import Solver
    else:
        from bin.train_linglf02mel_seq2seq_encAddlf0 import Solver
    mode = 'train'
    solver = Solver(config, paras, mode)
    solver.load_data()
    solver.set_model()
